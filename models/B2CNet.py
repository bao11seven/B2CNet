...
class BFAM_x(nn.Module):
    def __init__(self,inp,out):
        super(BFAM_x, self).__init__()
    ...
    def forward(self,inp1,inp2,last_feature):
        x = torch.cat([inp1,inp2],dim=1)
        c1 = self.conv_1(x)
        c2 = self.conv_2(x)
        c3 = self.conv_3(x)
...
class B2CNet(nn.Module):
    def __init__(self):
        ...
        self.bfam_1 = BFAM_x(128, 32)
        self.bfam_2 = BFAM_x(256, 64)
        self.bfam_3 = BFAM_x(512, 128)
